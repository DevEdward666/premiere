...
import {
  ACTION_GET_DEVICE,
  action_GET_notications, ACTION_NOTIF, signalr_connection,
...
function BottomNavigation() {
  ...
  const [logoImage, setlogoImage] = useState(require("../assets/icons/PremiereIcon2.png"));
    ...
    const createHubConnection = () => {
      if(mounted){
        ...
        hubconnectnotify.on('notifytoreact',  (message) => {
            ...
            if ((message?.from !== '' && message?.to === user_details?.prem_id) ||message?.to === 'all'
          ) {
            dispatch(
              ACTION_NOTIF(
